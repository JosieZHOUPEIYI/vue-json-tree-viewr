  ...
  <div :class="jvClass">
    <div v-if="copyable" class="jv-tooltip">
      <span ref="clip" class="jv-button" :class="{copied}">
        <slot name="copy" :copied="copied">{{ copied ? copyText.copiedText : copyText.copyText }}</slot>
      </span>
    </div>
    <div class="jv-code" :class="{'open': expandCode, boxed}" 
    >
      <json-box ref="jsonBox" :value="value" :sort="sort" :jsonStringExpand="jsonStringExpand" :highlightMouseoverNode="highlightMouseoverNode" :deep="deep" />
    </div>
